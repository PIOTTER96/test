...
throw -666;
 ...
 void Druzyna::createGracz(int i)
 {
     for(int a=0;a<=i;i++)
     {
         this->czlonkowie.push_back(new Gracz(Kibic.losujimie(),Kibic.losujnazwisko(),Kibic.losujwiek()));
     }
 }
void Druzyna::createTrener(int i)
{
    for(int a=0;a<=i;i++)
     {
        this->trenerzy.push_back(new Trener(losujimie(),losujnazwisko(),losujwiek()));
     }
}
void Druzyna::wypiszTrener(int i)
{
    for(int a=0;a<=i;i++)
     {
       czlonkowie.at(i)->show();
     }
}
void  Druzyna::wypiszGracze(int i)
{
    for(int a=0;a<=i;i++)
     {
        trenerzy.at(i)->show();
